...
The repository is split between two directories. The grafting-custom-gpu-binaries holds all the work I did for CSCI 32, during which I learned to reverse engineer GPU binaries (Nvidia ```.fatbin``` files) and interpret/write SASS, CUDA (C++), and PTX. Ultimately, I was able to bypass the CUDA compiler. The tutorial for how to achieve what I accomplished is titled "Grafting Custom GPU sections (.cubin files) into a fatbin file & Executing fatbins with the Nvidia Driver API".
